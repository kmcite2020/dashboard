...
List<DailyPrayer> get dailyPrayers => dailyPrayersRM.state;
void setList(index, whichPrayer, bool value) {
  dailyPrayersRM.state = [];
}
...
List<DailyPrayer> computation() {
  var reqDur = Duration();
  var nonPrayDur = Duration(days: 14 * 365);
  var now = DateTime.now();
  var dob = dateOfBirthRM.state;
  var dop = dateOfPubertyRM.state;
  if (dateOfPubertyRM.state != null) {
    reqDur = now.difference(dop!);
  } else {
    if (now.difference(dob) > nonPrayDur) {
      reqDur = now.difference(dob);
    } else {
      print('null');
    }
  }
...
final dailyPrayersRM = RM.injectFuture(
  () async => compute((message) => computation(), null),
  dependsOn: DependsOn(
    {
      dateOfBirthRM,
      dateOfPubertyRM,
...
final dateOfBirthRM = RM.inject<DateTime>(() => DateTime(2008, 11, 26));
final dateOfPubertyRM = RM.inject<DateTime?>(() => null);
